...
High school: {\bf Korean Minjok Leadership Academy (KMLA)} \hfill \textit{Feb. 2014} 
...
\begin{rSection}{Research Interests}
	I have backgrounds in machine learning (reinforcement learning, large-scale learning) and data mining (graph mining). I am interested in machine learning methods that could solve real-world problems.
\end{rSection}
...
\hfill \textit{Mar. 2020 - Current}\\
\textit{Supporter: KAIST}
\begin{itemize}
	\item (i) Developed methods for inferring the daily infected cases of COVID-19 from the daily confirmed cases. (ii) Developed epidemic models to understand the spread of the virus. Currently, I am studying ways to combine (i) and (ii) in order to jointly improve prediction/inference accuracy.
\end{itemize}	
	
\textbf{Application services using power consumption data}
\hfill \textit{Aug. 2018 - Mar. 2020}\\
\textit{Supporter: Korea Electric Power Corporation}
\begin{itemize}
	\item Developed a deep learning-based system for real service that provides predictions of open/close times of commercial facilities. The prediction was made by learning from power consumption data in the form of time series. 
	\item One of the challenges came from noisy and missing data. We tackled this problem by applying various machine learning and data processing techniques, such as data cleaning and aggregation, unsupervised/semi-supervised learning, and crowdsourcing.
\end{itemize}
...
\begin{itemize}
	\item Developed algorithms that recommended a keyword among a large candidate set per day. The goal was to find the keyword (or query) that would produce the highest satisfaction of users if its results page were updated. (The update mechanism was fixed and given.)
	\item Handled the scalability issue of deep reinforcement learning methods. If the action space was given as the set of all keywords, DQN performed more poorly than greedy methods. We solved this issue by parameter sharing, an idea that eventually became a research published in IJCAI.
...
\begin{itemize}
	\item Made drones learn to follow a moving target. The target was captured by a built-in camera. There were two sub-tasks: image recognition and target tracking. I focused on the target tracking part and developed algorithms based on reinforcement learning. We first worked on the simulated environment and then ran drones in the field.
	\item Developed a deep learning-based energy consumption model for drones. Given the state of a drone (e.g., velocity, altitude, pitch and roll), the task was to infer its energy consumption level. The purpose was to insert my model within the larger system of target tracking, such that the drones may learn long-term energy-efficient paths.
\end{itemize}
...
\begin{itemize}
	\item Studied and implemented methods for computer aided diagnoses of breast cancer. I applied various models to classify mammograms as cancer or benign, and applied image processing methods to improve prediction quality.
\end{itemize}
...
\begin{itemize}
	\item Participated in a project for AI-based automatic surveillance systems. The goal was to build a deep model that detected fire whenever it was caught on a security camera. My task was to improve the accuracy of the neural network inferring the existence of fire within an image.
\end{itemize}
...
Python (Pytorch, Tensorflow), Julia, Matlab, C, C++, Verilog  \\
