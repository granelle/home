...
\begin{itemize}
	\item Developed methods for inferring the daily infected cases of COVID-19 from the daily confirmed cases. Modified classic epidemic models to better understand the spread of the virus.
\end{itemize}	
...
\begin{itemize}
	...
	\item One of the challenges came from noisy and missing data. We tackled this problem by applying various machine learning and data processing techniques, such as data cleaning and aggregation, unsupervised/semi-supervised learning, clustering, and crowdsourcing.
\end{itemize}
...
\begin{itemize}
	\item Developed algorithms that recommended a keyword among a large candidate set per day. The goal was to find the keyword (or query) that would produce the highest satisfaction of users if its results page were updated. 
	\item Handled the scalability issue of deep reinforcement learning methods. If the action space was given as the set of all keywords, DQN performed more poorly than greedy methods. The issue was solved by parameter sharing, which we further developed into a research paper in IJCAI.
